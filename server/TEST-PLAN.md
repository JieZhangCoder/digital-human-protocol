...
| 角色 | 本地 | 企业 |
|---|---|---|
| Registry URL | `http://127.0.0.1:18181` | 例 `http://<registry-host>:18081` |
...
| 客户端 product.json | 从 `halo-local/product.enterprise-local.json` 复制 | CI 在出包时 `cp product.enterprise.json → product.json` |
...
cp halo-local/product.enterprise-local.json product.json

# 启
npm run dev
```

**第一屏期望**：登录页，看到 "Acme Corp" 一键登录按钮（绿色背景，⚡ 图标）作为推荐项。这证明 `product.json` 加载生效。
...
- store 一片空白 → DevTools Network 看请求是否打到 `127.0.0.1:18181`，如果打到 `<registry-host>` 说明 product.json 没刷新（重启客户端）
...
把所有 `127.0.0.1:18181` 换成生产 URL（如 `<registry-host>:18081`）、把 `enterprise-local-test` 换成真 token。其余步骤一字不改。跑完 12 步全通 = 部署成功。
