...
本文档描述如何在**单台开发机上**模拟一次完整的企业部署（如 Acme Corp 的 `<registry-host>:18081`），跑通：
...
### 2.1 准备客户端（覆盖 `product.enterprise.json`）

在 Halo 仓库的 `feature/app-market` 分支上，把 `product.enterprise.json` 临时改一下，让它指向本机 Registry：

```diff
   "registryOverrides": {
     "official": {
-      "url": "http://<registry-host>:18081",
+      "url": "http://127.0.0.1:18181",
       "name": "Enterprise Digital Human Registry",
       "publish": {
...
启动 Registry（覆盖 token，匹配 product.enterprise.json）：
...
- 点 "Publish" 按钮 → 弹确认对话框
- 确认后客户端应：
  1. 通过 `publish.service` 读 `product.enterprise.json` 的 `publish.target=http-registry`
...
- 把 `product.enterprise.json` 的 `token` 临时改成 `wrong-token`，重启客户端
...
git checkout -- halo-local/product.enterprise.json    # 还原 token / URL
```
...
| 本地表现 | 企业部署等价物 |
|---|---|
| `dhp-registry --config=...yaml` 起在 `:18181` | 同样的二进制 + 同样的 yaml 起在 Acme Corp 内网 `<registry-host>:18081` |
| 客户端 `registryOverrides.official.url` 指向本地 | 企业版打包时 `product.enterprise.json` 已经写死内网 URL |
| 固定 token 在本地 yaml + product.json 同步 | 企业 ops 在部署流水线把同一个 token 同步进 yaml + 打包 product.json |
| AI judge 未配 → 4 条规则降级 warn | 配 Enterprise AI 网关 → 4 条规则真正生效 |
